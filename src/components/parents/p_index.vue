<template>
    <div class="wrap public">
      ...
      <div class="content">
        <div class="left">
          <div class="left_nav">
            <li v-for="(i,index) in this.navData" :class="pathindex == index?'active_nav':''">
              <div class="mainNav"  @click="isClick(index,i.status,i.path)">
              <img :src="i.icon"/>
            	<span>{{i.title}}</span>
              </div>
              <div class="fenNav" v-for="x in i.fuTitl" :class="fuNav == x.path?'fu_nav':''" v-if="i.status" @click="isClick2(index,x.path)">
                <i></i>
              	<span>{{x.title}}</span>
              </div>
            </li>
