    ...
    <div class="block">
        ...
        <div class="tableData">
            <el-table ref="singleTable" :data="tableData" highlight-current-row style="width: 100%" :summary-method="getSummaries" show-summary>
                ...
                <el-table-column label="卡金额" align="center">
                    <el-table-column property="mainWalletFill" label="主钱包" align="center"></el-table-column>
                    <el-table-column property="subsidyWalletFill" label="补贴钱包" align="center"></el-table-column>
                </el-table-column>
                <el-table-column property="useCount" label="使用次数" align="center">
                </el-table-column>
                <el-table-column prop="transactDate" label="补登时间" align="center">
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
        ...
        getData() {
            ...
            fetch.get(Allapi.fillreport, par, res => {
                if (res.code == 200) {
                    this.tableData = res.data.m1Writeoffs
                    this.total = res.data.sumCount
                    this.sumCount = res.data.sumCount
                    this.mainWalletSum = res.data.mainWalletSum
                    this.subsidyWalletSum = res.data.subsidyWalletSum
        ...
        getSummaries(param) {
            ...
            columns.forEach((column, index) => {
                if (index === 0) {
                    ...
                    sums[index] = '总笔数 ' + this.sumCount + ' 笔';
                    return;
                } else if (index === 3) {
                    sums[index] = this.mainWalletSum + '元';
                    return;
                } else if (index === 4) {
                    sums[index] = this.subsidyWalletSum + '元';
                    return;
                } else if (index === 5) {
                    sums[index] = '总金额 '+this.totalPrice + '元';
                    return;
                } else{
                    sums[index] = '';
                }
        ...
        exports(){
            fetch.export(Allapi.fillreportExcel, {...this.filter}, res => {})
        }
