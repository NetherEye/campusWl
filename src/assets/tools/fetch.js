...
class Fetch {
	...
	// export
	export1(url, data, callback, options) {
		let param = new FormData(); //创建form对象
		...
		axios({
			...
			.then(function(response) {
				checkStatusAndCode(response, callback, 'export', 'export1');
			})
			.catch(function(error) {
				if(axios.isCancel(error)){
					// 此处为切换路由中断上次请求的地方
	...
	// export
	exports(url, data, callback, options) {
		let param = new FormData(); //创建form对象
...
function checkStatusAndCode(response, callback, type , name){
	if(type == 'export'){  
	...
	if(response && ((response.status === 200 || response.status === 304 || response.status === 400))) {
		...
		if(response.data.code == 211) { // 后台自定义状态码非200时将message提醒给用户
			localStorage.clear()
			sessionStorage.clear()
			router.replace({name: '下线'})
		}
		return callback(response.data)
	}
}
